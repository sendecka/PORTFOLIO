/* Liczby na okladce pochodza z gotowej miary policzonej w Pythonie
   (build/compute_measures.py -> data/measures.js, MEASURES.home) */
(function fillHomeStats() {
  const fmtSpace = n => Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "\u00A0");
  const h = MEASURES.home;
  ...
  if (recEl) recEl.textContent = fmtSpace(h.totalRecords);
  if (empEl) empEl.textContent = fmtSpace(h.uniqueEmployees);
  if (hrsEl) hrsEl.textContent = fmtSpace(h.totalHours);
})();
