...
/* Style/kolory poziomow ryzyka - dane liczbowe (kto jest w jakim poziomie,
   Bradford Factor) pochodza z gotowej miary policzonej w Pythonie
   (build/compute_measures.py -> data/measures.js, MEASURES.risk). */
const TIER_STYLE = {
  Critical: { color:"#E24B4A", backBorder:"#FCEAEA", emoji:"\u{1F534}", range:"Bradford \u2265600", action:"require immediate action" },
  Urgent:   { color:"#BA7517", backBorder:"#FAEEDA", emoji:"\u{1F7E0}", range:"Bradford \u2265400", action:"formal meeting required" },
  Formal:   { color:"#C49A00", backBorder:"#FFF8E6", emoji:"\u{1F7E1}", range:"Bradford \u2265200", action:"scheduled interview needed" },
  Monitor:  { color:"#378ADD", backBorder:"#F0F4FF", emoji:"\u{1F535}", range:"Bradford \u2265100", action:"keep under observation" },
  Normal:   { color:"#639922", backBorder:"#EAF3DE", emoji:"\u{1F7E2}", range:"Bradford <100",  action:"no action required" },
};
const TIER_ORDER = ["Critical", "Urgent", "Formal", "Monitor", "Normal"];
...
/* ============ lista pracownikow z Bradford Factor - gotowa miara z MEASURES ============ */
const employees = MEASURES.risk.employees.map(e => ({ ...e, ...getProfile(e.id) }));
const totalEmp = MEASURES.risk.totalEmp;
const tierCountByKey = {};
MEASURES.risk.tierCounts.forEach(t => { tierCountByKey[t.key] = t; });

/* ============ tier KPI flip cards ============ */
document.getElementById("tierRow").innerHTML = TIER_ORDER.map(key => {
  const t = { key, ...TIER_STYLE[key] };
  const count = tierCountByKey[key].count;
  const pct = tierCountByKey[key].pct + "%";
  return `
