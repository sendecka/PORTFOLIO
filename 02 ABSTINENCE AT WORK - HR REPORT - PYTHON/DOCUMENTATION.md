...
- **Python (pandas)** — jednorazowy krok "build": liczy wszystkie wskaźniki
  (“miary”, analogicznie do miar DAX w Power BI) na podstawie CSV i zapisuje
  gotowy wynik do `data/measures.js`
- **Czysty HTML/CSS/JavaScript** w przeglądarce — brak frameworków, brak
  build stepu po stronie klienta. JS **nie liczy żadnych agregacji** — tylko
  odczytuje gotowe wartości z `MEASURES` i renderuje DOM/animacje/interakcje
...
- `js/pageN_x.js` — logika renderowania DOM i obsługi kliknięć filtrów tej
  strony (bez żadnych obliczeń na surowych danych)

Dodatkowo pliki współdzielone przez wszystkie strony:
- `build/compute_measures.py` — skrypt Python (pandas), który liczy
  wszystkie miary i zapisuje je do `data/measures.js` (patrz 3.3)
- `data/Absenteeism_at_work.csv` — źródłowe dane (740 rekordów)
- `data/measures.js` — gotowe, już policzone wyniki jako zmienna JS
  (`const MEASURES = {...}`), wygenerowane przez `compute_measures.py`
- `vendor/chart.umd.js` — biblioteka Chart.js (używana tylko przez stronę Summary)

### 3.3 Miary liczone w Pythonie, nie w przeglądarce

**Kluczowa decyzja architektoniczna tego projektu:** wszystkie wskaźniki
widoczne na stronach — sumy godzin, KPI, Bradford Factor, podziały wg wieku,
stażu, powodu absencji, sezonu, dnia tygodnia itd. — są liczone **raz, w
Pythonie** (`build/compute_measures.py`, pandas), a nie przy każdym
wczytaniu strony w przeglądarce. Zamysł jest analogiczny do miar DAX w
oryginalnym raporcie Power BI: miara jest zdefiniowana i policzona w jednym
miejscu, a warstwa prezentacji (tu: HTML/JS) tylko ją wyświetla.

Skrypt wczytuje `data/Absenteeism_at_work.csv` przez pandas, odtwarza 1:1
formuły, które wcześniej liczył JS (biny wieku/stażu/odległości, sezony wg
kalendarza PL, Bradford Factor, rankingi, itd. — patrz sekcja 6), i zapisuje
wynik jako pojedynczy obiekt `MEASURES` w `data/measures.js`:

```
MEASURES = {
  home: {...},                     // statyki na okladce
  summary: { byFilter: {...}, ... },     // stan "null" + 12 stanow miesiecy
  trend: { kpiByFilter: {...}, ... },    // stan "null" + 4 sezony + 5 dni
  rootcause: { byFilter: {...}, ... },   // stan "null" + 4 wieki + 5 stazy
  risk: { employees: [...], ... },       // lista 36 pracownikow z Bradford
  recommendations: {...},                // stale wskazniki strony 6
}
```

Interaktywne filtrowanie (klik w wykres/słupek) **nadal działa dokładnie
tak samo jak wcześniej** — różnica jest wyłącznie pod maską: zamiast filtrować
surowe wiersze i przeliczać agregaty w locie, JS po prostu odczytuje gotowy,
wcześniej policzony wynik dla danego stanu filtra (np.
`MEASURES.trend.kpiByFilter["season:Autumn"]`) i renderuje go. Każdy plik
`js/pageN_x.js` ma teraz sekcję renderującą (DOM, animacje, obsługa kliknięć)
ale **nie ma już** żadnej funkcji liczącej sumy/średnie/grupowania na
surowych rekordach.

Korzyści tego podejścia:
- **Jedno źródło prawdy dla formuł** — zmiana metodologii (np. progów
  Bradford Factor) wymaga edycji w jednym miejscu (Python), a nie osobno
  w każdym z sześciu plików JS
- **Mniejszy transfer danych** — `measures.js` (~27 KB, dane zagregowane)
  zamiast poprzedniego `data.js` (~320 KB, 740 surowych wierszy) ładowanego
  przez każdą z 6 stron
- Łatwiejsze testowanie/weryfikacja liczb (Python + pandas, nie ręczne
  odtwarzanie agregacji w JS)

### 3.3.1 Dlaczego dane są plikiem `.js`, a nie `.json` wczytywanym przez `fetch()`?
...
bezpieczeństwa (CORS) — próba wczytania pliku `.json` przez `fetch()` kończy
się błędem `Failed to fetch`.

Natomiast wczytywanie pliku przez `<script src="measures.js">` (tak jak
zwykły plik JS) **działa poprawnie nawet z dysku** — to inny mechanizm
przeglądarki, nieobjęty tym ograniczeniem. Dlatego `compute_measures.py`
zapisuje wynik jako plik `.js` definiujący zmienną globalną
(`const MEASURES = {...}`), a nie plik `.json` wczytywany asynchronicznie.
...
dwukrotnym kliknięciu `index.html` — bez serwera. Python jest potrzebny
tylko przy aktualizacji danych źródłowych (patrz sekcja 8.3), nie przy
codziennym przeglądaniu raportu.
...
js/
  ...
  pageN_x.js                    # Renderowanie DOM + obsluga klikniec filtrow.
                                 # ZERO obliczen/agregacji na surowych danych —
                                 # tylko odczyt gotowych wynikow z MEASURES.
build/
  compute_measures.py           # Skrypt Python (pandas) liczacy WSZYSTKIE
                                 # miary wyswietlane na kazdej stronie (zrodlo
                                 # prawdy dla formul — patrz sekcja 3.3)
  requirements.txt              # pandas
data/
  Absenteeism_at_work.csv       # Źródłowe dane (740 rekordów) — do wglądu/edycji
  measures.js                   # Gotowe wyniki z compute_measures.py jako JS
                                 # (const MEASURES = {...}; ladowane przez <script src>)
vendor/
...
(`js/pageN_x.js`) oraz do wspólnych `data/measures.js` i (dla Summary)
`vendor/chart.umd.js`. Można otworzyć dowolny plik z `pages/` bezpośrednio w
przeglądarce, niezależnie od `index.html` — pod warunkiem, że `data/measures.js`
już istnieje (patrz sekcja 8.3, jeśli go brakuje lub dane się zmieniły).
...
- **Brak zapisu stanu filtrów** — odświeżenie strony lub przejście między
  ...
  filtra).
- **Statyczne dane** — wszelkie zmiany w danych źródłowych wymagają
  ponownego uruchomienia `python3 build/compute_measures.py` (patrz sekcja
  8.3); brak połączenia z bazą danych ani API, wymaga zainstalowanego
  Pythona + pandas do przeliczenia (samo przeglądanie gotowego raportu — nie).
- **Filtry ograniczone do z góry policzonych stanów** — interaktywne
  filtrowanie (klik w wykres) działa dla dyskretnego zbioru stanów
  wyliczonych w Pythonie (np. 12 miesięcy, 4 sezony, 5 dni tygodnia, 4 biny
  wieku, 5 binów stażu — patrz `compute_measures.py`). Nie da się dodać
  nowego wymiaru filtrowania bez rozszerzenia skryptu Python o odpowiednie
  stany i przeliczenia.
- **Rozmiar danych przy skalowaniu** — `data/measures.js` (~27 KB) jest
  dużo mniejszy niż poprzednie podejście z surowymi wierszami (~320 KB),
  bo zawiera już zagregowane wyniki, nie 740 pojedynczych rekordów. Przy
  znacznie większym zbiorze źródłowym (dziesiątki tysięcy wierszy) sam plik
  CSV i czas działania `compute_measures.py` rosłyby, ale rozmiar
  `measures.js` pozostałby zbliżony (bo zależy od liczby kategorii/stanów
  filtrów, nie od liczby wierszy źródłowych).
